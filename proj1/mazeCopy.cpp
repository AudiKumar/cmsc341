...
std::vector<int> Maze::solve() const {
  ...
  while (dest != getNcell()-1 && s.empty() == false){
    ...
    cout << "\nsrc:   " << src << endl;
    cout << "dest:  " << dest  << endl;
    //cout << "neighbhors" << _mazeCopy[dest].neighbors<< endl;

    
    if (n[0] > VACANT && n[0] != src ){
      src = dest;
      dest = n[0];
    ...
    //reached when you need to pop stacks
      ...
      do{          
        ...
        //int nIndex = getNIndex(src, dest, _mazeCopy);
        ...
        if (s.empty() == false){
         ...
         // _mazeCopy[src].neighbors[nIndex] = VACANT;
          ...
          if(_mazeCopy[newDest].neighbors[0] > VACANT && _mazeCopy[dest].neighbors[0] != newSrc ){
            ...
            cout << "code shouldn't execute" << endl;
          }
